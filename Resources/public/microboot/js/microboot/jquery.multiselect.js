...
(function ($) {
	$.widget("ui.multiselect", {
		...
		_create: function () {
			...
			this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><label class="icon-search" for="' + this.id + '-search"></label><input id="' + this.id + '-search" type="text" class="search empty ui-widget-content ui-corner-all"/>' + (this.options.includeAddAll ? '<a href="#" class="add-all">' + $.ui.multiselect.locale.addAll + '</a>' : '<span class="add-all">&nbsp;</span>') + '</div>').appendTo(this.availableContainer);

			this.selectedList = $('<ul class="selected connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function () { return false; }).appendTo(this.selectedContainer);
			this.availableList = $('<ul class="available connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function () { return false; }).appendTo(this.availableContainer);
			this.insertIcons = $('html').hasClass('ie7') ? true :  false;
		...
		_applyItemState: function (item, selected) {
			if (selected) {
				if (this.options.sortable) {
					item.children('span').addClass('icon-move-v').removeClass('ui-helper-hidden').addClass('ui-icon');
				}
				else {
					item.children('span').removeClass('icon-move-v').addClass('ui-helper-hidden').removeClass('ui-icon');
				}

				if (this.insertIcons) {
					item.find('a.action span').addClass('icon-subtract-js').removeClass('icon-add-js').html(microboot.icon('icon-subtract'));
				}
				else {
					item.find('a.action span').addClass('icon-subtract').removeClass('icon-add');
				}

				this._registerRemoveEvents(item.find('a.action'));
			} else {
				if (this.insertIcons) {
					item.find('a.action span').addClass('icon-add-js').removeClass('icon-subtract-js').html(microboot.icon('icon-add'));
				}
				else {
					item.find('a.action span').addClass('icon-add').removeClass('icon-subtract');
				}
