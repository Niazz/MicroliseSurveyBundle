	...
	$.fn.jqGrid = function (pin) {
		...
		return this.each(function () {
			...
			var p = $.extend(true, {
				...
				autowidth: true,
				scrollOffset: 18,
				cellLayout: 21, // = (@tableCellPaddingHorizontal * 2) + border-width
				subGridWidth: 20,
				multiselectWidth: 20,
				gridview: false,
				rownumWidth: 25,
		...
		isMSIE = $.browser.msie ? true : false;
			...
			$("<div class='ui-jqgrid-loading ui-state-default ui-state-active' id='load_" + this.id + "'><div class='loading'><i class='icon icon-loading icon--large icon--spin' aria-hidden='true'></i> " + this.p.loadtext + "</div></div>").insertBefore(gv);
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
			...
			// result += (clas !== undefined ? (" class=\""+clas+"\"") :"") + ((cm.title && tv) ? (" title=\""+$.jgrid.stripHtml(tv)+"\"") :""); Destroyd TD titles
		...
		addJSONData = function (data, t, rcnt, more, adjust) {
			...
			while (i < len) {
				...
				if (ts.p.gridview === false) {
					...
					$("#" + $.jgrid.jqID(ts.p.id)).find("tbody:first").append(rowData.join('')); // Override: fixes IE7 and IE8 bug where certain grids crash and never load
		...
		setPager = function (pgid, tp) {
			...
			if (ts.p.pgbuttons === true) {
				...
				pgl += "<td id='" + po[0] + "' class='ui-pg-button ui-corner-all'><i class='icon-first icon'></i></td>";
				pgl += "<td id='" + po[1] + "' class='ui-pg-button ui-corner-all'><i class='icon-back icon'></i></td>";
				pgl += pginp !== "" ? sep + pginp + sep : "";
				pgl += "<td id='" + po[2] + "' class='ui-pg-button ui-corner-all'><i class='icon-forward icon'></i></td>";
				pgl += "<td id='" + po[3] + "' class='ui-pg-button ui-corner-all'><i class='icon-last icon'></i></td>";
			} else if (pginp !== "") { pgl += pginp; }

			pgl += "</tr></tbody></table>";
			if (ts.p.rowList.length > 0) {
				$("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append("<div dir='" + dir + "' style='text-align:" + ts.p.recordpos + "' class='ui-paging-info'>" + str + " " + ts.p.resultsNumber + "</div>");
			}
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
			...
			imgs = "<span class='s-ico' style='display:none'><span sort='asc' class='icon-up icon ui-grid-ico-sort ui-icon-asc" + iac + " ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-" + dir + "'></span>";
			imgs += "<span sort='desc' class='icon-down icon ui-grid-ico-sort ui-icon-desc" + idc + " ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-" + dir + "'></span></span>";
				...
				if (typeof sort !== 'boolean') {
					...
					if (!(nm == 'cb' || nm == 'subgrid' || nm == 'rn')) {
						if (ts.p.viewsortcols[2]) {
							// Sortable is true
							if (ts.p.colModel[j].sortable === true) {
								$(this).addClass('jqgrid-sortable'); // Styling hook
								$(">div", this).addClass('ui-jqgrid-sortable'); // Functionality hook
							}
						}
					}
				}
	...
	$.jgrid.extend({
		...
		viewGridRow: function (rowid, p) {
			...
			maxCols = 1, maxRows = 0;
				...
				tbl = $("<table id='" + frmtb_id + "' class='EditTable' cellspacing='1' cellpadding='0' border='0' style='table-layout:fixed'><tbody></tbody></table>");
					if (onBeforeInit) {
						showFrm = onBeforeInit.call($t, $("#" + frmgr));
						if (typeof (showFrm) == "undefined") {
							showFrm = true;
		...
		navGrid: function (elem, o, pEdit, pAdd, pDel, pSearch, pView) {
			...
			$t = this, twd, tdw;
				...
				for (var i = 0; i < clone; i++) {
					var tbd,
				navtbl = $("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable'><tbody><tr></tr></tbody></table>"),
